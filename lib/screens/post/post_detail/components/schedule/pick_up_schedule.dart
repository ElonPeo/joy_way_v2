...
class PickupSchedule extends StatefulWidget {
  ...
  final int index;
  const PickupSchedule({super.key, required this.passenger, required this.index});
...
class _PickupScheduleState extends State<PickupSchedule> {
  ...
  Widget build(BuildContext context) {
    ...
    final depParts = LocationNameHandling.splitPlaceParts(_req!.pickUpName ?? '');
    final depFull = depParts.join(', ');

    final timeStr = TimeProcessing.formatHourMinute(_req!.desiredPickUpTime) ?? '';
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child:  Center(
                      child: ImageIcon(
                        const AssetImage("assets/icons/post/pick-up.png"),
                        size: 18,
