...
class _AvatarAndBackgroundImageState extends State<AvatarAndBackgroundImage> {
  ...
  Future<void> _loadCurrentUserImages() async {
    final imgs = await ProfileFireStorageImage()
        .getCurrentUserAvatarAndBackgroundUrls(); // <-- dùng hàm cho current user
  ...
  Future<void> _pickAvatar() async {
    ...
    setState(() => _isProcessingAvatar = true);        // << thêm
    try {
      ...
      final normalized = await ImageProcessing().normalizeToJpeg(
        original, targetW: 1280, targetH: 720, quality: 70,
      );

      if (normalized == null) {
        if (!mounted) return;
        ShowNotification.showAnimatedSnackBar(context, 'Unable to normalize image (invalid file).', 0, const Duration(milliseconds: 500));
        return;
      }
      if (!mounted) return;
  ...
  Future<void> _pickBackground() async {
    ...
    try {
      ...
      final original = File(picked.path);
      final normalized = await ImageProcessing().normalizeToJpeg(
        original, targetW: 1920, targetH: 1080, quality: 75,
      );

      if (normalized == null) {
        if (!mounted) return;
        ShowNotification.showAnimatedSnackBar(context, 'Unable to normalize background image (invalid file).', 0, const Duration(milliseconds: 500));
        return;
      }
      if (!mounted) return;
  ...
  ImageProvider _bgProvider() {
    ...
    if (_bgUrl != null && _bgUrl!.isNotEmpty) return NetworkImage(_bgUrl!); // rồi tới URL
    return const AssetImage('assets/background/backgroundEX.jpg');    // fallback asset
  }
              ...
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    width: 100,
                    decoration: BoxDecoration(
                        color: specs.backgroundColor, shape: BoxShape.circle),
                    child: Center(
                      child: ClipRRect(
                        ...
                        child: SizedBox(
                          height: 96,
                          width: 96,
                          child: Image(image: _avatarProvider(), fit: BoxFit.cover)
                        ),
                      ),
                    ),
                  ),
